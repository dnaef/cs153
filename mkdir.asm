...
main(int argc, char *argv[])
{
  ...
  for(i = 1; i < argc; i++){
    ...
    1025:	e8 f6 02 00 00       	call   1320 <mkdir>
  ...
  for(i = 1; i < argc; i++){
    ...
    104a:	c7 44 24 04 8d 17 00 	movl   $0x178d,0x4(%esp)
    1051:	00 
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    ...
    13aa:	f7 f7                	div    %edi
    13ac:	0f b6 92 b0 17 00 00 	movzbl 0x17b0(%edx),%edx
...
putc(int fd, char c)
{
  write(fd, &c, 1);
    ...
    1540:	8b 55 d4             	mov    -0x2c(%ebp),%edx
        ap++;
        if(s == 0)
    1543:	b9 a9 17 00 00       	mov    $0x17a9,%ecx
        ap++;
      } else if(c == 'x' || c == 'p'){
        printint(fd, *ap, 16, 0);
        ap++;
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    1601:	a1 cc 17 00 00       	mov    0x17cc,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
  ...
  freep = p;
    1651:	a3 cc 17 00 00       	mov    %eax,0x17cc
}
    1656:	5b                   	pop    %ebx
    1657:	5e                   	pop    %esi
    1658:	5f                   	pop    %edi
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    167d:	a3 cc 17 00 00       	mov    %eax,0x17cc
}
    1682:	5b                   	pop    %ebx
    1683:	5e                   	pop    %esi
    1684:	5f                   	pop    %edi
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    169c:	8b 0d cc 17 00 00    	mov    0x17cc,%ecx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
  ...
  if(nu < 4096)
    ...
    if(p == freep)
    16d0:	3b 05 cc 17 00 00    	cmp    0x17cc,%eax
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    16ef:	89 58 04             	mov    %ebx,0x4(%eax)
      ...
      freep = prevp;
    16f2:	89 0d cc 17 00 00    	mov    %ecx,0x17cc
      return (void*)(p + 1);
...
morecore(uint nu)
{
  ...
  return freep;
    1737:	8b 0d cc 17 00 00    	mov    0x17cc,%ecx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    174e:	c7 05 cc 17 00 00 c4 	movl   $0x17c4,0x17cc
    1755:	17 00 00 
    base.s.size = 0;
    1758:	b9 c4 17 00 00       	mov    $0x17c4,%ecx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    175d:	c7 05 c4 17 00 00 c4 	movl   $0x17c4,0x17c4
    1764:	17 00 00 
    base.s.size = 0;
    1767:	c7 05 c8 17 00 00 00 	movl   $0x0,0x17c8
    176e:	00 00 00 
